...
- `Prix moyen` reste le cout moyen IBKR. Le `Prix d'entree` essaie d'abord d'etre reconstruit depuis les executions IBKR disponibles, puis retombe sur l'historique d'ordres Bubo (`data/logs/orders.jsonl`) si besoin; le `P/L` ouvert et le `P/L total` sont recalcules avec le prix live IBKR et cette entree.
...
services:
  bubo-web:
    ...
    environment:
      ...
      BUBO_UNIVERSE_MUTE_MAX_HOURS: ${BUBO_UNIVERSE_MUTE_MAX_HOURS:-336}
...
| `BUBO_UNIVERSE_MUTE_MAX_HOURS` | Duree maximale de pause avec backoff (anti-retry infini sur ticker invalide) | Non | Entier `>= BUBO_UNIVERSE_MUTE_HOURS` | `336` |
